...
def find_all_paths(graph, start, end, path, all_paths, prefix=b'', suffix=b''):
    path = path + [(start, prefix, suffix)]
    if start == end:
        all_paths.append(path)
        return
    for node in graph[start]:
        preimage = g.preimages[node]
        offset = 0
        while True:
            offset = preimage.find(start, offset)
            if offset == -1:
                break
            prefix = preimage[:offset]
            suffix = preimage[offset + len(start):]
            find_all_paths(graph, node, end, path, all_paths, prefix, suffix)
            offset += len(start)
    return all_paths
...
def print_paths(paths):
    ...
    for path in paths:
        ...
        print(len(path) - 1)
        for _, prefix, suffix in path[1:]:
            if len(prefix) > max_bytes:
                max_bytes = len(prefix)
            if len(suffix) > max_bytes:
                max_bytes = len(suffix)
            print(prefix.hex())
            print(suffix.hex())
